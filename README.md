...
![CodeReview Agent 首页](docs/images/home-dashboard.png)

> 截图使用浏览器内健康接口桩展示就绪态，仅用于说明界面，不代表生产部署。
...
不替代人工审查，也不宣称替代成熟 SAST。当前不提供 Docker Compose 或演示视频，
本地运行依赖已有 PostgreSQL、pgvector 和 Redis。

## 架构

```mermaid
flowchart LR
    U["Vue 3 Web"] --> A["FastAPI"]
    A --> P["PostgreSQL + pgvector"]
    A --> R["Redis"]
    A --> C["Celery worker"]
    C --> S["Scanner + LanguageAdapter"]
...
所有 LangGraph 路由都是纯函数。耗时审查由 Celery 执行，HTTP 只创建任务；任务写入
使用幂等键，SSE 以数据库事件 ID 支持断线续传。
